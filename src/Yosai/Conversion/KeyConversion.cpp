    ...
    namespace {
        ...
        public:
            ...
            void insert(T t, const std::string &str) {
                m_string_vector[t + mSpecialValueCount] = str;
                m_T_map[str] = t;
            }

            const std::string &at(T t) const {
                auto index = static_cast<std::size_t>(t + mSpecialValueCount);
                assertion(index < m_string_vector.size() + mSpecialValueCount, "No match for key / mouse button found");
